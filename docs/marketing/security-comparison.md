...
| **5. Restrict administrative privileges** (least-privilege, no daily-use admin accounts) | ✅ Mature (JWT-protected dashboard; four-role RBAC: owner/staff/visitor/developer; developer sandbox-isolated; CSRF + rate limit; unique credential enforcement planned) | 🔴 Immature (default `admin` username; weak passwords common; no MFA by default) | ✅ Mature (scoped staff roles; platform-enforced) | ✅ Mature (staff accounts with granular permissions) | ⚠️ Partial (limited admin roles; SSPR often requires upgrade) |
