...
description: >
  Design system composed of 9 orthogonal, independently variant design dimensions.
  Each dimension has A/B/C spec files. Bundles group coherent defaults. URL params
...
The dimension-based design system replaces monolithic theme files with 9 independent design dimensions, each stored as a standalone JSON spec file with A/B/C variants. At request time, a bundle config (or URL overrides) selects the active variant per dimension, specs are loaded, compiled to CSS custom properties, and applied to `:root` via `ThemeProvider`. This avoids combinatorial explosion — only 27 spec files (9 dimensions x 3 variants) are needed to produce any of 19683 possible combinations.
...
Parses URL search parameters and optional bundle configs to resolve the active variant (A, B, or C) for each of the 9 dimensions.
...
5. If override is valid (A, B, or C), set that dimension's variant
...
For each of the 9 dimensions, if a spec exists, compile it:
...
| Page-Layout defaults | `compilation.page-layout.*` | Fallback hero/nav/section/card/footer variants |
...
| Business rules | Bundle files are JSON in `{bundles_root}/{id}.json`. A bundle assigns one variant (A, B, or C) to every dimension. |
...
Verifies that all variant pairs of each dimension (A/B, A/C, B/C) differ by a minimum threshold, ensuring generated themes feel perceptibly distinct.
...
| Page-Layout hero default | `compilation.page-layout.default_hero_variant` | fullscreen | Fallback hero layout variant |
| Page-Layout nav default | `compilation.page-layout.default_nav_variant` | top-bar | Fallback navigation layout variant |
| Page-Layout section default | `compilation.page-layout.default_section_container` | alternating | Fallback section container style |
| Page-Layout card default | `compilation.page-layout.default_card_variant` | elevated | Fallback card visual variant |
| Page-Layout footer default | `compilation.page-layout.default_footer_variant` | columns | Fallback footer layout variant |
...
After creating, modifying, or adding any dimension spec files — including adding entirely new
dimensions — run the `theme-uniqueness` skill to ensure the changes haven't introduced
hardcoded values, silent fallbacks, or identical variant pairs:

```
skills/theme-uniqueness/SKILL.md
```

This audits 5 layers (status colors, flat Tailwind, white/black, dimensions, fallbacks),
verifies all variant pairs differ in every dimension and component property, and validates
no dead CSS vars or silent fallbacks remain. Run it during development whenever the dimension
system changes.
...
| `dimension-page-layout.md` | Hero layout, nav variant, section/card/footer style |
