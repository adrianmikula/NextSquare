...
- Blocks can specify a `layout` field for compositional page structure. See `CmsBlock` schema for available layout values (`full-width`, `half-width`, `two-thirds`, `sidebar-content`, `card-grid`, `full-bleed`). Default is `full-width` if omitted.
- When generating page-layout dimension specs, you can set `componentOverrides` to select alternative React components per block type. This allows themes to swap component implementations without code changes. See `skills/theme-dimensions/resources/dimension-page-layout.md` for schema details.
